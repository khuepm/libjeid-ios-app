...
/// <code>DLFiles</code>オブジェクトをシリアライズします
///
/// throws:
...
/// <code>EPFiles</code>オブジェクトをシリアライズします
///
/// throws:
/// エンコードに失敗した場合、エラーがスローされます
///
...
- (nonnull instancetype)init:(JeidReader * _Nonnull)reader OBJC_DESIGNATED_INITIALIZER;
/// 内部認証を有効化/無効化します
/// \param flag 内部認証を有効化/無効化フラグ
///
- (void)setEnableInternalAuthentication:(BOOL)flag;
...
/// \param readMyNumber trueでマイナンバーを読み出します。デフォルトでtrueです。
///
/// \param readAttributes trueで4属性情報を読み出します。デフォルトでtrueです。
///
///
/// throws:
/// 読み出しに失敗した場合、エラーがスローされます
...
/// returns:
/// 券面入力補助AP内のすべてのデータ
- (INTextFiles * _Nullable)readFiles:(BOOL)readMyNumber :(BOOL)readAttributes error:(NSError * _Nullable * _Nullable)error SWIFT_WARN_UNUSED_RESULT;
/// 個人番号を取得します
/// note:
/// 無償版の場合は<code>JeidError.unsupportedOperation(message:)</code>がスローされます
///
...
/// <code>INVisualFiles</code>オブジェクトをシリアライズします
///
/// throws:
/// エンコードに失敗した場合、エラーがスローされます
///
...
/// <code>RCFiles</code>オブジェクトをシリアライズします
///
/// throws:
/// エンコードに失敗した場合、エラーがスローされます
///
